...
import type { SystemResearchExportOptions } from '@/types/system-research';

type ExportFormState = {
    format: SystemResearchExportOptions['format'] | '';
    dateRange: SystemResearchExportOptions['dateRange'] | '';
    startDate: string;
    endDate: string;
...
type ExportRecordsModalProps = {
    ...
    onExport: (options: SystemResearchExportOptions) => Promise<void>;
};
...
const statusOptions: Array<{ key: keyof ExportFormState; label: string }> = [
    { key: 'includePublished', label: 'Published' },
    { key: 'includeUnderReview', label: 'Under Review' },
    { key: 'includeDraft', label: 'Draft' },
    { key: 'includeArchived', label: 'Archived' },
];

const sectionOptions: Array<{ key: keyof ExportFormState; label: string }> = [
    { key: 'includeBasicInfo', label: 'Basic Research Information' },
    { key: 'includeAuthors', label: 'Authors and Contributors' },
    { key: 'includeAgencyInfo', label: 'Agency Information' },
    { key: 'includeMetadata', label: 'Abstract and Metadata' },
    { key: 'includeSDGs', label: 'SDG Tags' },
    { key: 'includeUsageStats', label: 'Views and Downloads' },
    { key: 'includeAccessType', label: 'Access Type' },
    { key: 'includeDates', label: 'Upload/Publication Dates' },
];
...
export function ExportRecordsModal({
    ...
    const validate = () => {
        ...
        if (!statusOptions.some((option) => form[option.key] === true)) {
            nextErrors.statuses = 'Select at least one record status.';
        }
    ...
    const handleExport = async () => {
        ...
        await onExport({
            ...
            includeBasicInfo: form.includeBasicInfo,
            includeAuthors: form.includeAuthors,
            includeAgencyInfo: form.includeAgencyInfo,
            includeMetadata: form.includeMetadata,
            includeSDGs: form.includeSDGs,
            includeUsageStats: form.includeUsageStats,
            includeAccessType: form.includeAccessType,
            ...
            <DialogContent className="rounded-[14px] border-[#e5e7eb] bg-white sm:max-w-[760px]">
                ...
                <div className="grid max-h-[70vh] gap-4 overflow-y-auto pr-1">
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                            <label
                                htmlFor="system-research-export-format"
                            ...
                            <Select
                                ...
                                onValueChange={(value) =>
                                    updateForm(
                                        'format',
                                        value as ExportFormState['format'],
                                    )
                                }
                            >
                                <SelectTrigger
                                    id="system-research-export-format"
                        ...
                        <div>
                            <label
                                htmlFor="system-research-export-range"
                                className="text-sm font-semibold text-[#1e2939]"
                            >
                                Date Range
                            </label>
                            <Select
                                ...
                                onValueChange={(value) =>
                                    updateForm(
                                        'dateRange',
                                        value as ExportFormState['dateRange'],
                                    )
                                }
                            >
                                <SelectTrigger
                                    id="system-research-export-range"
                                    className="mt-2 h-10 w-full rounded-[10px] border-[#e5e7eb]"
                                >
                    ...
                    {form.dateRange === 'custom' ? (
                        <div className="grid gap-4 rounded-[12px] border border-[#e5e7eb] bg-[#f9fafb] p-4 sm:grid-cols-2">
                            <div>
                                <label
                                    htmlFor="system-research-export-start-date"
                                    className="text-sm font-semibold text-[#1e2939]"
                                >
                                    Custom Start Date
                                </label>
                                <input
                                    id="system-research-export-start-date"
                                    type="date"
                            ...
                            <div>
                                <label
                                    htmlFor="system-research-export-end-date"
                                    className="text-sm font-semibold text-[#1e2939]"
                                >
                                    Custom End Date
                                </label>
                                <input
                                    id="system-research-export-end-date"
                                    type="date"
                                    value={form.endDate}
                                    onChange={(event) =>
                                        updateForm(
                    ...
                    <label className="flex items-center gap-2 rounded-[12px] border border-[#e5e7eb] bg-white p-4 text-sm text-[#4a5565]">
                        <Checkbox
                            ...
                            className="border-[#d1d5dc] data-[state=checked]:border-[#1e3a8a] data-[state=checked]:bg-[#1e3a8a]"
                        />
                        Include current filter selection in export
                    </label>
                </div>
