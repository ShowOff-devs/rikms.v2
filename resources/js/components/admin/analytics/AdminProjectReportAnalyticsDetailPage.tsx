...
export function AdminProjectReportAnalyticsDetailPage({
    researchId,
}: {
    researchId: string;
}) {
    ...
    useEffect(() => {
        ...
        getAdminProjectReportDetail(researchId, controller.signal)
            .then((data) => {
                if (!isCurrent) {
                    return;
                }
    ...
    return (
        <>
            <Head title="Admin Project Report Analytics Detail" />

            <AdminLayout search={search} onSearchChange={setSearch}>
                <main className="px-4 py-8 lg:px-8">
                    <div className="mx-auto flex max-w-[1230px] flex-col gap-5">
    ...
    return (
        <section className="rounded-[14px] border border-[#e5e7eb] bg-white p-5 shadow-[0px_1px_3px_0px_rgba(0,0,0,0.08)]">
            ...
            <p className="mt-4 text-xs font-medium tracking-wide text-[#6a7282] uppercase">
                Super Admin Project Report Analytics
            </p>
            ...
            <p className="mt-2 text-sm text-[#6a7282]">
                Read-only regional report metrics and persisted performance
                data.
            </p>
        </section>
    );
...
function InfoPanel({ data }: { data: ProjectReportAnalyticsDetail }) {
    return (
        <Panel
            ...
            description="Identity, agency, workflow, and period details."
        >
...
function AccomplishmentPanel({ data }: { data: ProjectReportAnalyticsDetail }) {
    return (
        <Panel
            ...
            description="Official percentage and backend classification."
        >
            <div className="grid gap-3 sm:grid-cols-2">
                <MetricBox
                    label="Official Percentage"
...
function BudgetPanel({ data }: { data: ProjectReportAnalyticsDetail }) {
    ...
    return (
        <Panel
            ...
            description="Read-only backend-calculated budget metrics."
        >
            <div className="grid gap-3 sm:grid-cols-2">
                <MetricBox
                    label="Allotted"
...
function PerformanceRowsPanel({
    ...
    return (
        <Panel
            ...
            description="Activity, target, actual, accomplishment, status, and remarks."
        >
            {rows.length === 0 ? (
