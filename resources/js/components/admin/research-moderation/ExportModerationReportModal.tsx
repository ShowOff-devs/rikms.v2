...
import type { ModerationReportExportOptions } from '@/types/research-moderation';

type ExportFormat = ModerationReportExportOptions['format'];
type ExportDateRange = ModerationReportExportOptions['dateRange'];
...
const initialExportState: ExportFormState = {
    ...
    includeFlaggedRecords: true,
    includePendingReview: true,
    includeResolvedIssues: true,
    includeDuplicateAlerts: true,
...
const sectionOptions: Array<{ key: keyof ExportFormState; label: string }> = [
    ...
    { key: 'includeFlaggedRecords', label: 'Flagged Research Records' },
    { key: 'includePendingReview', label: 'Pending Review Items' },
    { key: 'includeResolvedIssues', label: 'Resolved Issues' },
    { key: 'includeDuplicateAlerts', label: 'Duplicate Research Alerts' },
    { key: 'includePolicyViolations', label: 'Policy Violations' },
    { key: 'includeActivityLog', label: 'Moderation Activity Log' },
];

export function ExportModerationReportModal({
    open,
    isExporting,
    onOpenChange,
    onExport,
    ...
    onExport: (options: ModerationReportExportOptions) => Promise<void>;
}) {
    ...
    const handleExport = async () => {
        ...
        await onExport({
            ...
            includePendingReview: form.includePendingReview,
            includeResolvedIssues: form.includeResolvedIssues,
            includeDuplicateAlerts: form.includeDuplicateAlerts,
            includePolicyViolations: form.includePolicyViolations,
            ...
            <DialogContent className="rounded-[14px] border-[#e5e7eb] bg-white sm:max-w-[680px]">
                ...
                <div className="grid max-h-[70vh] gap-4 overflow-y-auto pr-1">
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                            <label
                                htmlFor="moderation-export-format"
                            ...
                            <Select
                                value={form.format}
                                onValueChange={(value) =>
                                    updateForm('format', value as ExportFormat)
                                }
                            >
                                <SelectTrigger
                                    id="moderation-export-format"
                        ...
                        <div>
                            <label
                                htmlFor="moderation-export-range"
                                className="text-sm font-semibold text-[#1e2939]"
                            >
                                Date Range
                            </label>
                            <Select
                                ...
                                onValueChange={(value) =>
                                    updateForm(
                                        'dateRange',
                                        value as ExportDateRange,
                                    )
                                }
                            >
                                <SelectTrigger
                                    id="moderation-export-range"
                                    className="mt-2 h-10 w-full rounded-[10px] border-[#e5e7eb]"
                                >
                    ...
                    {form.dateRange === 'custom' ? (
                        <div className="grid gap-4 rounded-[12px] border border-[#e5e7eb] bg-[#f9fafb] p-4 sm:grid-cols-2">
                            <div>
                                <label
                                    htmlFor="moderation-export-start-date"
                                    className="text-sm font-semibold text-[#1e2939]"
                                >
                                    Custom Start Date
                                </label>
                                <input
                                    id="moderation-export-start-date"
                                    type="date"
                            ...
                            <div>
                                <label
                                    htmlFor="moderation-export-end-date"
                                    className="text-sm font-semibold text-[#1e2939]"
                                >
                                    Custom End Date
                                </label>
                                <input
                                    id="moderation-export-end-date"
                                    type="date"
                                    value={form.endDate}
                                    onChange={(event) =>
                                        updateForm(
