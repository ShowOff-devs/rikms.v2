...
| CAPTCHA configuration and backend verification | Implemented but awaiting staging | `config/rikms.php`, `.env.example`, verifier and production validation tests | Test real Turnstile keys/domains, missing/invalid token, provider outage | HTTPS staging | Security/application | Complete live-provider verification |
| CSP staged enforcement | Implemented but awaiting staging | `config/security_headers.php`, middleware/tests, `docs/CSP_DEPLOYMENT.md` | Exercise documented routes in report-only mode and review collected violations | HTTPS staging | Security/application | Configure report collector; resolve evidence; approve enforcement |
...
| Automated backup execution | Not implemented | `docs/BACKUP_FEATURE_STATUS.md`; operational test confirms no backup route/helper | Inspect routes/services; verify UI remains informational | Repository | Operations/product | Select and implement or formally provision external backup automation |
...
| Monitoring and alert delivery | Documentation/configuration only | Signals and recovery steps in `docs/OPERATIONS_RUNBOOK.md` | Configure dashboards/alerts and inject controlled failures | Ubuntu staging/monitoring | Operations/security | Prove alert delivery, ownership, and response |
...
6. CAPTCHA frontend/backend flags, key boundaries, production validation, and failure behavior were aligned.
...
6. Configure Turnstile staging keys/domain and test valid, missing, invalid, expired, and provider-unavailable cases.
